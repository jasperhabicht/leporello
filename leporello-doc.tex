...
\subsection{RTL typesetting}

The document class natively supports right-to-left typesetting with LuaLaTeX. Since the \macro{babel} package also makes use of the native functions for bidirectional typesetting provided by LuaLaTeX, the package will work best with a combination of LuaLaTeX and \macro{babel}.

If the \macro{bidi} package is used (which is used by the \macro{polyglossia} package), the following addition to the preamble is needed:
