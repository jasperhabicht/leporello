...
\warning Currently, the \macro{box/stretch} key only affects the outer box. Therefore, if a box is stretched and has an offset, the height (or width in vertical typesetting) needs to be given explicitly as the text will flow out of the box otherwise.
...
box/|no padding|
\end{macrodef}
\macro{box/padding left}, \macro{box/padding right}, \macro{box/padding top} and \macro{box/padding bottom} each expect a dimension to describe the padding of the contents from the relevant edge of the box. If not specified, the default value of \macro{7.5mm} is assumed.
...
The package uses the hook \macro{begindocument/before} to insert layout-related settings as well as the hook \macro{shipout/foreground} to insert code to draw crop marks. It uses the hook \macro{enddocument} to insert code for the automatic typesetting mechanism that is activated via the \macro{global/auto typeset} key. It also uses the hook \macro{label} to change the default code for the \macro{\label} so that it immediately writes to the auxiliary file.
...
This example shows some features of the flow mechanism. The text in the second box is moved to the end of the collected contents of all three boxes that are connected via the \macro{box/flow into} and \macro{box/name} keys. Note that the contents are appended without adding anything between, which is why a \macro{\par} was added in this example. This example further shows how the command \macro{\leporelloboxbreak} can be used to ensure that text that flows across boxes with different width is typeset correctly.
