...
\newlength\oparindent
\NewDocumentCommand{\raggedrightindent}{}{%
  \setlength\oparindent{\parindent}%
  \raggedright%
  \setlength\parindent{\oparindent}%
}
...
\end{macrodef}
If set, the widths of the columns are reversed in every second page. This key is set to \macro{true} per default. Use \macro{alternate layout=false} to disable this function. 
...
Typeset material can flow from one box to another box. To this end, the key \macro{flow into} can be set to the relevant box and assigned the name of the box to flow into as value. The name of the relevant box can be set via the \macro{name} key. If two or more boxes are connected this way, contents of the boxes are always concatenated.

Prior to shipping out a (PDF) page, all material in boxes that have the \macro{flow into} key set (and where another box with the relevant \macro{name} exists) is collected and shipped out to pages of the size of the boxes the relevant material should flow into. The resulting boxes are saved internally and the shipout of these boxes is discarded. The saved boxes are then placed into the relevant boxes on the (PDF) page which is eventually shipped out. 
...
The package uses the hook \macro{begindocument/before} to insert layout-related settings as well as the hook \macro{shipout/foreground} to insert code to draw cropmarks. It uses the hook \macro{enddocument} to insert code for the automatic typesetting mechanism that is activated via the \macro{auto typesetting} key. It also uses the hook \macro{label} to change the default code for the \macro{\label} so that it immediately writes to the auxiliary file.
...
{\raggedrightindent The hooks \macro{leporello/break-flow/before}, \macro{leporello/typeset/before} and \macro{leporello/box/begin} are used to set and unset right-to-left and vertical typesetting mode.\par}

{\raggedrightindent Finally, command and environment hooks are used to issue warnings if the commands \macro{\footnote}, \macro{\footnotemark} and \macro{\footnotetext} as well as the environments \macro{figure} or \macro{table} are used as this document class does not support these.\par}
...
\end{macrodef}
{\raggedright These variables hold the relevant values of the \macro{typeset/reverse layout}, \macro{typeset/reverse order}, \macro{typeset/reverse pagination}, \macro{typeset/alternate layout} and \macro{typeset/continuous pagination} keys. \par}
...
\begin{macrodef}
|\l_leporello_current_sheet_int|
|\l_leporello_current_column_int|
|\l_leporello_current_box_int|
\end{macrodef}
These variables hold the ID of the current sheet (page in the PDF file), the current column (column on the sheet) and the current box as integer respectively.

\begin{macrodef}
|\g_leporello_current_page_int|
\end{macrodef}
This variable holds the current page. A page is considered a column in the document. Pagination is not necessarily continuous. Note that the current page number is always set globally.
